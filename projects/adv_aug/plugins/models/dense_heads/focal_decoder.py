...
from projects.adv_aug.plugins.models.utils.utils import MLP, gen_sineembed_for_position, gen_sineembed_for_position_all
from projects.adv_aug.plugins.models.utils.transformer import *
from projects.adv_aug.plugins.models.utils.decoder_utils import FFN
from projects.adv_aug.plugins.models.utils.time_utils import T
...
class FocalDecoder(nn.Module):
    ...
    def generate_gt_groups(self, query_feat, query_pos, query_heatmap_score, lidar_feat, lidar_feat_flatten, bev_pos, heatmap,
        ...
        flat_idx = batch_gt_pos[:, :, 1] * H + batch_gt_pos[:, :, 0]
        if flat_idx.min() < 0 or flat_idx.max() >= bev_pos.shape[1]:
            print("DEBUG ASSERT CRASH DETECTED!")
            print("flat_idx min:", flat_idx.min().item(), "max:", flat_idx.max().item())
            print("bev_pos shape:", bev_pos.shape)
            print("batch_gt_pos[:, :, 0] min:", batch_gt_pos[:, :, 0].min().item(), "max:", batch_gt_pos[:, :, 0].max().item())
            print("batch_gt_pos[:, :, 1] min:", batch_gt_pos[:, :, 1].min().item(), "max:", batch_gt_pos[:, :, 1].max().item())
            print("W:", W, "H:", H)
            # Clip index to be safe
            flat_idx = flat_idx.clamp(min=0, max=bev_pos.shape[1]-1)
        batch_gt_bev_pos = bev_pos.gather(index=flat_idx[...,None].expand(-1,-1, bev_pos.shape[-1]), dim=1)


        batch_gt_heatmap_score = heatmap.gather(index=flat_idx[:, None, :].expand(-1, self.num_classes, -1), dim=-1)
        batch_gt_query_feat = lidar_feat_flatten.gather(index=flat_idx[:, None, :].expand(-1, lidar_feat_flatten.shape[1], -1), dim=-1)
        ...
        if not self.multistage_heatmap:
            ...
            for i in range(self.multistage_heatmap):
                ...
                if self.heatmap_box:
                    ...
                    dense_heatmap_boxes_xy = dense_heatmap_boxes[:, :2, :, :] + bev_pos.int().float().transpose(1,2)[:, :, None].expand_as(dense_heatmap_boxes[:, :2, :, :])
                    dense_heatmap_boxes = torch.cat([dense_heatmap_boxes_xy, dense_heatmap_boxes[:, 2:]], dim=1)
